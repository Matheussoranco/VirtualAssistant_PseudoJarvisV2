...
def listen():
  ...
  with sr.Microphone() as source: 
    ...
    try:
      ...
      speak("Desculpe, não entendi Senhor")
    except sr.RequestError:
      speak("Desculpe, o serviço está offline Senhor")

    print("Jarvis>> ", voice_data.lower()) # print what user saidprint
    if voice_data:
      success = check_commands(voice_data)
      if not success:
        speak("Não consegui entender o comando, Senhor")
        ...
        for voice_command, command_action in command.items():
          ...
          if found_action:
            try:
              ...
              speak(f"Ocorreu um problema no comando {voice_command}. Repita, Senhor")
              return False
  return False
